...
export class IngredientsService {
  ...
  async createOrUpdate(dto: CreateIngredientDto, id_producto?: string) {
    try {
      const producto = id_producto
        ...
        : await this.prisma.db.producto.create({
            data: {
              pzas: dto.pzas,
              precio_medio: dto.precio_medio,
              cantidad_inventario: dto.cantidad_inventario,
      ...
      this.logger.log(`Ingrediente creado: ${JSON.stringify(producto)}`);
      return { message: "Ingrediente creado correctamente", ingrediente: producto };
    } catch (e: any) {
